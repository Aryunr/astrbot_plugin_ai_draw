...
class ContextMemory:
    ...
    def get_summary(self, user_id: str, group_id: str) -> str:
        ...
        lines = [f"你之前在本对话中帮用户做了 {len(rows)} 次图片处理（生成/识别）。如果用户追问相关内容，请自然回忆，不要提“系统”或“上下文记忆”等字眼："]
        ...
        for seq, rtype, prompt, result_text, created_at in rows:
            ...
            if rtype == "draw":
                desc = prompt[:100]
                lines.append(f'{seq}. ({ago}) 用户让你生成了一张图: "{desc}"')
            else:
                desc = result_text[:200] if result_text else prompt[:100]
                lines.append(f'{seq}. ({ago}) 用户发了一张图让你识别，你当时给出的识别结果是: "{desc}"')
    ...
    def _extract_keywords(self, query: str) -> List[str]:
        """从查询中提取有意义的搜索词。中文使用二元组分词，英文/数字保持原样。"""
        noise = {
            # 单字/口语词
            "的", "是", "我", "你", "他", "她", "它", "们", "了", "吗",
            "呢", "吧", "啊", "呀", "在", "有", "和", "与", "这", "那", "不",
            "就", "也", "都", "很", "要", "会", "能", "想", "说", "看",
            "来", "去", "上", "下", "里", "中", "个", "把", "被", "给",
            # 常见双字噪声（描述/查询场景下高频出现，避免干扰匹配）
            "一个", "一张", "一张图", "那张", "那张图", "图片", "什么", "里面",
            "描述", "来着", "这个", "那个", "一下", "帮我", "请问",
            "怎么", "这是", "那是", "可以", "知道", "告诉",
        }
        ...
        for seg in segments:
            if re.match(r'^[a-zA-Z0-9]+$', seg):
                # 英文/数字：保持原样
                if len(seg) >= 2 and seg.lower() not in noise:
                    keywords.append(seg)
            else:
                # 中文：拆成二元组，如"企鹅图片"→["企鹅","鹅图","图片"]
