...
class AiDrawPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        if self.config.get("context_memory_enabled", True):
            try:
                ...
                logger.error(f"AiDraw 上下文记忆初始化失败: {e}")
                self.context_memory = None
                ...
                if self.context_memory:
                    try:
                        self.context_memory.add(
                            ...
                            str(event.get_group_id()) if event.get_group_id() else "",
                            "describe",
                            "请描述图片内容",
                            image_desc,
                        )
    ...
    def _format_description(self, text: str) -> str:
        ...
        if max_len > 0 and len(text) > max_len:
            # 在 max_len 附近找自然断句点，避免硬截断导致信息丢失
            search_range = min(120, max_len)
            lookback_start = max(max_len - search_range, 0)
            segment = text[lookback_start:max_len]

            # 优先级：句末标点 > 换行 > 逗号/分号 > 空格 > 硬截断
            break_pos = -1
            for pattern in [r'[。！？](?!\w)', r'\n', r'[，；、]', r'\s']:
                matches = list(re.finditer(pattern, segment))
                if matches:
                    m = matches[-1]
                    break_pos = lookback_start + m.end()
                    break
        ...
        # 压缩多余空行（3+连续换行 → 双换行，保留段落感但去掉多余空白）
        text = _re.sub(r'\n{3,}', '\n\n', text)
        # 清除行首残留空格（Markdown标记移除后留下的缩进）
        ...
        if not image_comp:
            ...
            if self.context_memory:
                ...
                if seq_match:
                    ...
                    if search_text and not any(kw in search_text for kw in ["/draw", "/describe"]):
