...
# 将插件自身目录加入 sys.path，使 providers/ 子模块可被导入
...
# ============================================================================
# 日志过滤器：静默主代理的"处理图片描述失败"报错
# 原因：主代理在 OnLLMRequestEvent 派发之前就会尝试向不支持 vision 的 LLM
# 发送图片，导致 400 报错。此报错无法通过钩子拦截，故以日志过滤方式处理。
# ============================================================================

class _SuppressImageCaptionError(logging.Filter):
    """过滤 astr_main_agent 中"处理图片描述失败"的 ERROR 日志。

    该错误发生在主代理的图像字幕阶段（早于 OnLLMRequestEvent），
    无法通过插件钩子拦截，但功能上不影响本插件的正常识图流程。
    """
...
class PermissionManager:
    ...
    提供两级权限体系：
    - 管理员: 无调用限制
    - 普通用户: 每日调用次数上限（超出后静默拒绝）

    使用量数据持久化存储在 data/ai_draw/usage.json。
    """
...
class AiDrawPlugin(Star):
    ...
    async def on_agent_begin(self, event, *args, **kwargs):
        """在 Agent 开始处理时触发，早于 main agent 的图片提取。
        
        """
        if not _HAS_LLM_HOOK:
            return
        if not hasattr(event, 'message_obj') or not event.message_obj:
            logger.info("AiDraw on_agent_begin: event 无 message_obj，跳过")
            return
        if not hasattr(event.message_obj, 'message') or not event.message_obj.message:
            logger.info("AiDraw on_agent_begin: message_obj 无 message，跳过")
            return
        ...
        try:
            ...
            if sess in self._scene1_handled_sessions:
                # 从消息链中移除图片组件，防止 Agent 工具读取
                found, remaining = self._extract_images_recursive(event.message_obj.message)
                if found:
                    event.message_obj.message = remaining
                    # 如果有新图片（如引用消息），仍然处理识图
                    logger.info(f"AiDraw on_agent_begin: Scene 1 已处理但发现新图片，处理引用识图 (session={sess})")
                    await self._process_quote_images(event, found)
                    return
                logger.info(f"AiDraw on_agent_begin: Scene 1 已处理此会话 (session={sess})，跳过 Agent 处理")
                # 注意：不调用 stop_event()，以允许 Scene 0 文本追问正常走 LLM
                return
        except Exception:
            pass
        ...
        logger.info(f"AiDraw on_agent_begin: 已拦截 {len(found_images)} 张图片")

        # 直接在这里识图（作为 on_llm_request 的兜底）
        await self._process_quote_images(event, found_images)
        # 阻止 Agent 继续处理（_process_quote_images 内部已调用 stop_event）
        # 会话标记，防止重复处理
        try:
            sess = str(event.get_sender_id())
    ...
    async def _process_quote_images(self, event, found_images):
        ...
        if not image_url:
            ...
            if quoted_msg_id and quoted_msg_id in self._image_cache:
                ...
                logger.info(f"AiDraw on_agent_begin: 缓存命中，msg_id={quoted_msg_id}")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"AiDraw on_agent_begin: 已缓存识图结果 (session={session_id})")
            except Exception:
                pass
    ...
    def _clear_tool_images_cache(self) -> int:
        """清理 astrbot 工具图片缓存目录 (data/temp/tool_images/) 中的所有文件。
        
        防止 LLM Agent 通过 astrbot_file_read_tool 读取旧图片。
        Returns: 清理的文件数量。
        """
        try:
    ...
    def _clear_temp_image_files(self, temp_dir: str) -> int:
        """清理 data/temp/ 目录下 Agent 可能读取的原始图片文件。

        Agent 通过 astrbot_file_read_tool 可以直接读取原始文件（如 compressed_*.png），
        这些文件是 on_llm_request 处理后残留的。清理它们以防止 Agent 在后续阶段误读。
        Returns: 清理的文件数量。
        """
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req: ProviderRequest, *args, **kwargs):
        """在 LLM 请求前拦截，处理图片识别或文生图。
        
        """
        if not _HAS_LLM_HOOK:
            return

        logger.info(f"[AiDraw TRACE] on_llm_request called: {event.message_str[:100]!r}")
        ...
        if _is_cmd:
            ...
            if cmd_lower == "/test_draw_api":
                ...
                logger.info(f"[AiDraw TRACE] test_draw_api: calling stop_event(), is_stopped before={event.is_stopped()}")
                event.stop_event()
                logger.info(f"[AiDraw TRACE] test_draw_api: is_stopped after={event.is_stopped()}")
                return

            # /draw 和 /describe 由场景1/2自动处理，此处不做额外拦截


        # ---- 场景0: 无图片但有最近的分析缓存 → 注入上下文 ----
...
# ---- 场景2: 无图片，但有画图关键词 → 文生图 ----
        ...
        if "/" in user_message or _first_word in _cmd_names:
            logger.info(f"[AiDraw TRACE] Scene2: command detected, forcing should_draw=False: {user_message[:80]!r}")
            should_draw = False
        else:
